...
def asynchronize(motor_class, framework, sync_method, has_write_concern, doc=None):
    ...
    method.pymongo_method_name = mangle_delegate_name(motor_class, sync_method.__name__)
    if doc is not None:
        method.__doc__ = doc
    return method
...
class WrapBase(AttributeFactory):
...
class WrapAsync(WrapBase):
    ...
    def create_attribute(self, cls, attr_name):
        ...
        wrapper = cls._framework.pymongo_class_wrapper(async_method, original_class)
        if self.doc:
...
class Unwrap(WrapBase):
    def __init__(self, prop, motor_class_name):
        ...
        assert isinstance(motor_class_name, pycompat.text_type)
    ...
    def create_attribute(self, cls, attr_name):
        ...
        def _f(self, *args, **kwargs):
            ...
            unwrapped_kwargs = dict(
                [(key, obj.delegate if obj.__class__.__name__ == name else obj) for key, obj in kwargs.items()])
...
class ReadOnlyPropertyDescriptor(object):
    ...
    def __get__(self, obj, objtype):
        if obj:
            if hasattr(obj.delegate, self.attr_name):
            ...
            if hasattr(objtype.__delegate_class__, self.attr_name):
